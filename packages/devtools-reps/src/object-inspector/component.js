...
class ObjectInspector extends Component {
  ...
  setExpanded(item: Node, expand: boolean) {
    ...
    if (expand === true) {
      nodeExpand(item, loadedProperties, createObjectClient);
    } else {
      nodeCollapse(item);
    }
  }
  ...
  renderTreeItemLabel(label) {
    if (label === null || typeof label === "undefined") {
      return null;
    }

    return dom.span({className: "object-label"}, label);
  }
  ...
  renderTreeItem(
    ...
    const labelElement = this.renderTreeItemLabel(label);
